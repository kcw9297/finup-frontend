...
import { useYoutubeDetail } from "../hooks/useYoutubeDetail";
...
export default function YoutubeEdit() {
  const { videoId } = useParams();
  const { detailRp } = useYoutubeDetail(videoId);
  // [1] 훅 불러오기
  const {
  ...
  useEffect(() => {
    ...
    changePreviewRq({ videoUrl: detailRp.videoUrl })
    changeYoutubeWriteRq({
        ...
        <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
          ...
          <TextField
            ...
            onChange={(e) => {
              ...
              changeYoutubeWriteRq({ videoUrl: e.target.value });
            }}
          />
