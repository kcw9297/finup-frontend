...
export function useStudyWordList({ admin = false }) {
  ...
  const handleSearch = e => {
    ...
    const nextRq = { ...searchRq, keyword: curKeyword || '', filter: curFilter || searchRq.filter, pageNum: DEFAULT_SEARCH_RQ.pageNum }; // 키워드 반영
  ...
  const handleFilter = filter => {
    ...
    // 다음 요청
    const nextRq = { ...searchRq, filter, pageNum: DEFAULT_SEARCH_RQ.pageNum, keyword: curKeyword };
